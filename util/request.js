...
const chooseUserAgent = ua => {
  ...
  let index = 0;
  if (typeof ua == 'undefined') index = Math.floor(
    Math.random() * userAgentList.length); else if (ua ===
    'mobile') index = Math.floor(Math.random() * 7); else if (ua ===
    'pc') index = Math.floor(Math.random() * 5) + 8; else return ua;
...
const createRequest = (method, url, data, options) => {
  return new Promise((resolve, reject) => {
    ...
    if (url.includes(
      'music.163.com')) headers['Referer'] = 'https://music.163.com';
    // headers['X-Real-IP'] = '118.88.88.88'

    if (typeof options.cookie === 'object') headers['Cookie'] = Object.keys(
      options.cookie).
    ...
    if (options.crypto === 'weapi') {
      ...
      data = encrypt.linuxapi({
        method: method, url: url.replace(/\w*api/, 'api'), params: data,
      });
