  ...
  <div class="container" ref="container">
    ...
    <div class="top">
      <div class="left">
        ...
        <ul class="select" ref="select" v-show="showSelect" @click="showSelect = false">
          <li v-for="(item, index) in types" :key="index" @click="currentSelect = index">
            {{ item.name }}
          </li>
        </ul>
      </div>
      <div class="right">
        <yd-search v-model="searchKeywords" :on-submit="submitHandler" placeholder="点击回车搜索"> </yd-search>
      </div>
    </div>
    <div class="tips" v-show="searchKeywords.length === 0">输入你喜欢的爱豆~ 可选搜索类型哦~</div>
    <div class="list">
...
export default {
  ...
  methods: {
    async getSearch(keywords, type, page) {
      const [error, result] = await catchError(api.getSearch(keywords, type, page));
      if (error) {
      ...
      if (result.code === 200) {
        ...
        if (Object.keys(result.result).length === 0 || (result.result.songs && !result.result.songs.length) || (result.result.playlists && !result.result.playlists.length)) {
